...
int main(int argc, char* argv[]) {
    ...
    unsigned int* device_trans_array;
    cuda_ret = cudaMalloc((void**)&device_trans_array, n_transactions * sizeof(unsigned int));
    err_check(cuda_ret, (char*)"Unable to allocate transactions to device memory!", 1);

	// Get transactions from host memory
    cuda_ret = cudaMemcpy(device_trans_array, transactions, n_transactions * sizeof(unsigned int), cudaMemcpyHostToDevice);
    err_check(cuda_ret, (char*)"Unable to read transactions from host memory!", 3);
    ...
    err_check(cuda_ret, (char*)"Unable to allocate hashs to device memory!", 1);
	
    // Launch the hash kernel
    hash_kernel <<< dimGrid, dimBlock >>> (
        device_hash_array,   // put hashs into here
		device_nonce_array,  //put nonces into here 
        trials,              // size of array
		device_trans_array,  //put trans into here 
        n_transactions,      // size of trans
		MAX                  // to mod with
        );
    cuda_ret = cudaDeviceSynchronize();
    ...
    reduction_kernel <<< dimGrid, dimBlock, 2 * BLOCK_SIZE * sizeof(int) >>> (
        device_hash_array,   // put hashs into here
		device_nonce_array,  //put nonces into here 
        trials,              // size of array
		MAX                  // to mod with
    ...
    unsigned int* hash_array = (unsigned int*)calloc(num_blocks, sizeof(unsigned int));
    cuda_ret = cudaMemcpy(hash_array, device_hash_array, num_blocks * sizeof(unsigned int), cudaMemcpyDeviceToHost);
    err_check(cuda_ret, (char*)"Unable to read hashs from device memory!", 3);

    // Get nonces from device memory
    unsigned int* nonce_array = (unsigned int*)calloc(num_blocks, sizeof(unsigned int));
    cuda_ret = cudaMemcpy(nonce_array, device_nonce_array, num_blocks * sizeof(unsigned int), cudaMemcpyDeviceToHost);
